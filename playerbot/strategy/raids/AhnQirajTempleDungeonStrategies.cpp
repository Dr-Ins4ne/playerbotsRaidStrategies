...
void AhnQirajTempleDungeonStrategy::InitCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode("end skeram fight", NextAction::array(0, new NextAction("disable skeram fight strategy", 100.0f), NULL)));
}
...
void SkeramFightStrategy::InitCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode("skeram tank out of position", NextAction::array(0, new NextAction("skeram pull to tank position", 101.0f), NULL)));
